...
2.0 版本把这个工作台从“能接上桌面”推进到“能日常管理桌面工作流”：新增移动首页和项目选择、分支选择和 worktree/PR 草稿、归档箱、额度面板、设置页、可信设备管理、终端配对入口、PWA 更新提示、真实 UI 截图和更完整的安全边界。

> 本项目为二开，在 fork 版的基础上，深度适配了 Docker 容器部署场景，移除了部分功能，同时大幅优化性能，提高在容器环境工作的稳定性，修复了原版的部分 bug ，提高了安全性，优化了前端界面并提供更丰富的功能。
...
- **Live mirror 工作流**：移动端不只是发起新对话，还能看到 Codex 正在运行、等待确认、后台执行、已完成的真实状态，并继续 steer、queue、interrupt。
...
- PWA 更新提示：检测到新的前端构建后，移动端会提示刷新到新版。
...
- `CODEXMOBILE_PAIRING_CODE_TTL_MS`：一次性配对码有效期，默认 10 分钟
- `CODEXMOBILE_PAIRING_CODE_LENGTH`：一次性配对码长度，默认 10 位
...
- `CODEX_HOME`：Codex 配置目录，默认 `~/.codex`（Docker 默认 `/app/.codex`）
- `CODEXMOBILE_HOME`：CodexMobile 本地状态目录，默认 `.codexmobile/state`（Docker 默认 `/app/.codexmobile/state`）
- `CODEXMOBILE_WORKDIR`：CodexMobile 工作目录，默认当前目录（Docker 默认 `/workspace`）。**文件浏览器只能访问此目录及默认工作目录**
...
文件浏览器 API（`/api/files/*`、`/api/local-file`）**只能访问允许的工作目录**：

- `CODEXMOBILE_WORKDIR` 环境变量指定的目录
- `defaultProjectlessWorkspaceRoot()` 返回的默认工作目录

访问其他路径会返回 403 错误。这防止了通过文件 API 访问系统敏感文件。

不要提交 `.env`、`.codexmobile`、证书、日志、上传文件、生成图片或本地认证数据。

## 快速开始

CodexMobile 推荐 Docker 容器化部署，适合在服务器或 NAS 上运行。

### 使用 Docker Compose（推荐）

```bash
# 创建数据目录
mkdir -p ~/codex ~/codexmobile ~/workspace
...
docker run -d \
  ...
  -p 3321:3321 \
  -v ~/codex:/app/.codex \
  -v ~/codexmobile:/app/.codexmobile \
  -v ~/workspace:/workspace \
  -e OPENAI_API_KEY=your-api-key \
  ghcr.io/flyyangx/codexmobile:latest
```
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `CODEX_HOME` | `/app/.codex` | Codex 配置目录 |
| `CODEXMOBILE_HOME` | `/app/.codexmobile/state` | CodexMobile 状态目录 |
| `CODEXMOBILE_WORKDIR` | `/workspace` | 工作目录 |
...
| 卷 | 说明 |
|----|------|
| `/app/.codex` | Codex 配置和会话数据 |
| `/app/.codexmobile` | CodexMobile 状态、上传文件、生成图片 |
| `/workspace` | Codex 项目代码存放目录 |
| `/app/.env` | 环境变量文件 |
...
- `npm run pair`：向本机服务申请一次性配对码，在终端输出手机链接
